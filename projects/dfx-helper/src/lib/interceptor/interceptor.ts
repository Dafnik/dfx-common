...
export function shouldIntercept(
  ...
  BY_PASS?: HttpContextToken<boolean>,
  ignorePaths?: string[],
): boolean {
...
export function baseUrlInterceptor(
  ...
  if (
    baseUrl.length > 0 &&
    shouldIntercept(req, BASE_URL_INTERCEPTOR, inject(HELPER_BASE_URL_INTERCEPTOR_IGNORE_PATHS))
  ) {
...
export function postPutJsonContentTypeInterceptor(
  req: HttpRequest<unknown>,
  next: HttpHandlerFn,
): Observable<HttpEvent<unknown>> {
  if (
