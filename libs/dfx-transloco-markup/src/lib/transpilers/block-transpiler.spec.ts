    ...
    it('recognizes block boundaries in translations', () => {
      ...
      for (const { translation, offset, expectToken } of testCases) {
        ...
        if (expectToken) {
          ...
          expect(result!.nextOffset - offset, `expected tokenize('${translation}', ${offset}) to return a token of length 3`).toBe(3);
        } else {
          expect(result, `expected tokenize('${translation}', ${offset}) to return undefined`).toBeUndefined();
        }
      }
    });
  });
  ...
  describe('transpile function', () => {
      ...
      for (const [offset, expectedResult] of expectedResults.entries()) {
        ...
        if (expectedResult === 0) {
          ...
          expect(
            ...
            `expected transpile(tokens, ${offset}, context).nextOffset - offset to be ${expectedResult}`,
          ).toBe(expectedResult);
        }
      }
    });
    ...
    it('can handle syntax errors', () => {
      ...
      const testCases = [
        ...
        {
          tokens: [new BlockBoundary('<<<'), new BlockBoundary('<<<')],
      ...
      for (const { tokens, expectedNextOffset } of testCases) {
        ...
        if (expectedNextOffset === 0) {
          ...
          expect(result!.nextOffset, `tokens.length = ${tokens.length}`).toBe(expectedNextOffset);
        }
      }
    });
    ...
    it('recursively transpiles its content', () => {
      ...
      const testCases = [
        {
          tokens: [start],
          expectedRecursiveTranspileOffsets: [],
        },
        {
          tokens: [end, start],
          expectedRecursiveTranspileOffsets: [],
        ...
        {
          tokens: [start, 0, end],
          expectedRecursiveTranspileOffsets: [1],
        },
        {
      ...
      for (const { tokens, expectedRecursiveTranspileOffsets } of testCases) {
        const context = new TranslationMarkupTranspilerContext(tokens, {}, [transpiler]);
        const rootTranspilerSpy = vi.spyOn(context, 'transpile');

        transpiler.transpile(0, context);

        const actualRecursiveTranspileOffsets = rootTranspilerSpy.mock.calls.map((call) => call[0]);
