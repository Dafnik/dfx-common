...
function createTestTranspiler(
  ...
  return new ContextualLinkBlockTranspiler(
    ...
    TestBed.inject(TranslationMarkupRendererFactory),
...
describe('ContextualLinkBlockTranspiler', () => {
  ...
  describe('transpile function', () => {
    ...
    it('uses the provided link renderers to render the links', () => {
      ...
      const transpiler = createTestTranspiler('[', ']', [stringLinkRenderer, externalLinkObjectLinkRenderer], { parameterKey: 'testLink' });
    ...
    it('supports static links', () => {
      ...
      renderLink({});
      expect(renderStringLinkSpy).toHaveBeenCalled();
      expect(renderStringLinkSpy.mock.calls.at(-1)?.[0]).toBe('test://foo');
    });
    ...
    it('supports parameter based links', () => {
      ...
      expect(renderStringLinkSpy).toHaveBeenCalled();
      expect(renderStringLinkSpy.mock.calls.at(-1)?.[0]).toBe('test://bar');
    ...
    it('supports dynamically resolved links', () => {
      ...
      const transpiler = createTestTranspiler('[', ']', [stringLinkRenderer], {
      ...
      expect(renderStringLinkSpy).toHaveBeenCalled();
      expect(renderStringLinkSpy.mock.calls.at(-1)?.[0]).toBe('test://baz');
    });
  });
});
