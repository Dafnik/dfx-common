...
describe('TranslationMarkupRendererFactory class', () => {
  describe('createTextRenderer function', () => {
    it('can be used to create a translation markup renderer for static text', () => {
      const rendererFactory = TestBed.inject(TranslationMarkupRendererFactory);
    ...
    it('can be used to create a translation markup renderer for dynamically resolved text', () => {
      ...
      expect(rendererFactory.createTextRenderer(({ something }) => `Apple ${something}`)({ something: '& Banana' }).textContent).toBe(
        'Apple & Banana',
      );

      expect(rendererFactory.createTextRenderer(({ a, b, c }) => `${a} ${b} ${c}`)({ a: 'Eat', b: 'some', c: 'fruit!' }).textContent).toBe(
        'Eat some fruit!',
      );
    });
  });
  ...
  describe('createElementRenderer function', () => {
    ...
    it('supports child renderers', () => {
      ...
      const render = rendererFactory.createElementRenderer('b', [
        ...
        rendererFactory.createElementRenderer('i', [rendererFactory.createTextRenderer(({ name }) => String(name))]),
