...
describe('bindProvider() function', () => {
  it('can bind unbound type providers', () => {
    TestBed.configureTestingModule({
      imports: [ServiceModule.withConfig(TestServiceImpl)],
    });
  ...
  it('can bind unbound class providers', () => {
    TestBed.configureTestingModule({
      imports: [ServiceModule.withConfig(useClass(TestServiceImpl))],
    });
  ...
  it('can bind unbound existing providers', () => {
    TestBed.configureTestingModule({
      ...
      providers: [TestServiceImpl, { provide: ALT_NUMBER_VALUE, useValue: 123 }, { provide: ALT_STRING_VALUE, useValue: 'test test' }],
    });
  ...
  it('supports optional providers', () => {
    TestBed.configureTestingModule({
      imports: [OptionalConfigModule.withConfig({})],
    });
  ...
  it('correctly handles falsy values for unbound value providers', () => {
    ...
    for (const test of tests) {
      ...
      if ('useValue' in unboundProvider) {
        ...
        throw new Error(`No \`UnboundValueProvider\` was returned for value ${test}`);
      }
    }
  });
});
...
class ValueModule {
  public static withConfig(options: ValueModuleOptions): ModuleWithProviders<ValueModule> {
    return {
      ...
      providers: [bindProvider(NUMBER_VALUE, options.numberValue), bindProvider(STRING_VALUE, options.stringValue)],
    };
  }
}
...
class ServiceModule {
  public static withConfig(service: UnboundProvider<TestService>): ModuleWithProviders<ServiceModule> {
    return {
      ...
      providers: [bindProvider(TestService, service)],
    };
  }
}
...
class MultiValueModule {
  public static withConfig(numberValue: UnboundProvider<number>): ModuleWithProviders<MultiValueModule> {
    return {
      ngModule: ValueModule,
      providers: [bindProvider(NUMBER_VALUE, numberValue, { multi: true })],
    };
  }
}
