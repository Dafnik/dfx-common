...
**dfx-transloco-markup** offers an alternative to the Transloco [directive](https://jsverse.gitbook.io/transloco/core-concepts/translation-in-the-template#structural-directive) and [pipe](https://jsverse.gitbook.io/transloco/core-concepts/translation-in-the-template#pipe): the `<transloco>` component, that takes care of rendering your translations with markup.
...
If this is not the case follow the easy [installation instructions](https://jsverse.gitbook.io/transloco/getting-started/installation) from Transloco.

Next you'll need to install the `dfx-transloco-markup` package from NPM:
...
```shell
pnpm install dfx-transloco-markup
```
...
After having installed the `dfx-transloco-markup` package you can now add support for translation markup rendering to your application.
...
@Component({
  ...
  template: `
    <transloco [key]="'GREETING'" [params]="{ name: firstName + ' ' + lastName, website: 'https://www.example.com/' }"></transloco>
  `,
})
...
- **`lang`** (string)
  ...
  The default language is either the current language of the`TranslocoService`, or the language that was specified using the `TRANSLOCO_LANG` injection token.

- **`scope`** (string)

  Scope which is to be used.
  If this property is specified it will override the scope provide via the `TRANSLOCO_SCOPE` injection token.
...
As mentioned before, one of the features of `dfx-transloco-markup` is that you have full control over which transpilers will be available and at what level.
...
This is supported by **dfx-transloco-markup** by including `inheritTranslationMarkupTranspilers()` in the providers list of a module or component:

```typescript
import { inheritTranslationMarkupTranspilers } from 'dfx-transloco-markup';
...
Obviously `dfx-transloco-markup` will not recognize the `[product]` token by default, so you will need to create your own transpiler and make it [available](#defining-markup-transpiler-availability) to the `<transloco>` component.
...
Since the need for contextual link transpilers is quite common, `dfx-transloco-markup` provides a `ContextualLinkTranspilerFactory` to simplify their creation.
...
The whole architecture of `dfx-transloco-markup` is based on the concept of transpilers: small units that are responsible for converting (part) of translation values into rendering functions.
...
While the `dfx-transloco-markup` library ships with a set of standard transpilers for the most common use cases, there are surely a lot of other different markup requirements which are not supported out-of-the-box.
...
This is why `dfx-transloco-markup` converts translation values first into a sequence of tokens, before transpiling it into rendering functions.
...
A complete implementation of the colored text transpiler can be found in the demo application: [`apps/dfx-transloco-markup-demo/src/app/features/custom-transpilers/colored-text-transpiler.ts`](../../apps/dfx-transloco-markup-demo/src/app/features/custom-transpilers/colored-text-transpiler.ts)
...
One example is the [message format](https://jsverse.gitbook.io/transloco/plugins-and-extensions/message-format) plugin to support translations that use the ICU syntax for expressing pluralization and gender.

Transloco itself also uses a [transpiler](https://jsverse.gitbook.io/transloco/advanced-features/the-transpiler) for this purpose: the `TranslocoTranspiler`.
...
Since `dfx-transloco-markup` is meant to support all Transloco features, it also uses the `TranslocoTranspiler` to expand interpolation expressions.
This means that it also supports custom implementations, like the _message format_ plugin.

Due to the different transpiler architecture used by `dfx-transloco-markup`, you might need to help it
...
Custom interpolation expression can be recognized by `dfx-transloco-markup` if you provide a custom
...
To make the custom `InterpolationExpressionMatcher` implementation available to `dfx-transloco-markup` register it as provider in root module of your application using the `TRANSLATION_INTERPOLATION_EXPRESSION_MATCHER` injection token:

```typescript
import { TRANSLATION_INTERPOLATION_EXPRESSION_MATCHER } from 'dfx-transloco-markup';
...
**dfx-transloco-markup** supports two type of link models out-of-the-box: string values and values that conform to the `ExternalLink` model.
...
If your application uses the Angular router, then this is usually achieved by means of a [router link directive](https://angular.dev/api/router/RouterLink#).
...
```typescript
import { provideLinkRenderer } from 'dfx-transloco-markup';

@NgModule({
  providers: [provideLinkRenderer(MyCustomLinkRenderer)],
})
export class AppModule {}
```
